...
genimage_path()
{
	if [ -n "${BR2_EXTERNAL_PHYTEC_PATH}" ]; then
		echo "${BR2_EXTERNAL_PHYTEC_PATH}/board/freescale/common/imx/"
	else
		echo "board/freescale/common/imx/"
	fi
}

genimage_type()
{
	if grep -Eq "^BR2_TARGET_BAREBOX=y$" ${BR2_CONFIG}; then
		echo "genimage.cfg.template_barebox"
	elif grep -Eq "^BR2_PACKAGE_FREESCALE_IMX_PLATFORM_IMX8=y$" ${BR2_CONFIG}; then
...
uboot_image()
{
	if grep -Eq "^BR2_TARGET_UBOOT_FORMAT_DTB_IMX=y$" ${BR2_CONFIG}; then
	    ...
	    echo "u-boot.img"
	elif grep -Eq "^BR2_TARGET_BAREBOX=y$" ${BR2_CONFIG}; then
		grep -E "^BR2_TARGET_BAREBOX_IMAGE_FILE=*" ${BR2_CONFIG} | sed 's/.*="\(.*\)"/\1/g' | sed 's,.*/\(.*\)$,\1,'
	fi
}
...
GENIMAGE_PATH=$2
if [ -z "${GENIMAGE_PATH}" ]; then
  GENIMAGE_PATH=$(genimage_path)$(genimage_type)
fi
main $@
